...
{
  uint64_t clk_freq = 1000000; //getApbFrequency();   // <----------------------------modified
  ...
  return (clk_freq >> bit_num) / (double)div_num * 80;
}
...
static double _ledcTimerRead(uint8_t chan)
{
  ...
  clk_freq <<= 8;//div_num is 8 bit decimal
  return ((clk_freq >> bit_num) / (double)div_num) / 80; // <--------------------------modified: "/ 80" added
}
